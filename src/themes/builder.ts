    ...
    try {
      ...
      return sampleLog
        ...
        .map((line) => {
          ...
          const color =
            this.config.colors![level.toLowerCase()] ||
 ...
 */
export function generateThemeVariants(config: SimpleThemeConfig): {
  light: Theme;
  dark: Theme;
} {
...
export function validateTheme(theme: Theme): {
  ...
  textColors.forEach((color) => {
    ...
    if (ratio < 3) {
      ...
      errors.push(
        `Poor contrast ratio (${ratio.toFixed(2)}) between ${color} and ${bgColor}`,
      );
    } else if (ratio < 4.5) {
      contrastIssues.push({
        foreground: color,
      ...
      warnings.push(
        `Low contrast ratio (${ratio.toFixed(2)}) between ${color} and ${bgColor}`,
      );
    }
  });
...
export function adjustThemeForAccessibility(
  ...
  if (adjustedTheme.schema.matchWords) {
    Object.entries(adjustedTheme.schema.matchWords).forEach(
      ([_word, style]) => {
        if (
          style &&
          typeof style === "object" &&
          ...
          if (ratio < targetContrast) {
            ...
            if (analysis) {
              ...
              const newL = Math.max(
                0,
                Math.min(100, analysis.hsl.l + adjustment),
              );
...
export function checkWCAGCompliance(theme: Theme): {
  ...
  if (minRatio >= 7) {
    ...
    // AA for large text, A for normal
    results.level = "A";
  ...
  if (results.level === "FAIL") {
    results.recommendations.push(
      "Increase contrast ratios to at least 3:1 for basic accessibility",
    );
  }
  ...
  return {
    ...
    validateEnvironment(): {
      ...
      if (!hasColorTerm) {
        ...
        suggestions.push(
          "Set COLORTERM environment variable or use a color-capable terminal",
        );
      }
    ...
    detectConflicts(): string[] {
      ...
      colors.forEach((words, color) => {
        if (words.length > 3) {
          conflicts.push(
            `Color ${color} is overused (${words.length} elements): ${words.join(", ")}`,
          );
        }
      });
...
export function generateThemeReport(theme: Theme): {
  ...
  environmental: ReturnType<
    ReturnType<typeof createThemeMonitor>["validateEnvironment"]
  >;
  recommendations: string[];
  cssGeneration: {
    bem: string;
  ...
  const bgAnalysis = analyzeColor(
    theme.schema.defaultStyle?.color || "#ffffff",
  );
  const mode = bgAnalysis?.isDark
    ? "dark"
    : bgAnalysis?.isLight
      ? "light"
      : "unknown";
