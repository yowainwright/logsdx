...
function showBanner() {
  ...
  console.log(gradient.multiline(`
  ╦  ┌─┐┌─┐┌─┐╔╦╗═╗ ╦
  ║  │ ││ ┬└─┐ ║║╔╩╦╝
  ╩═╝└─┘└─┘└─┘═╩╝╩ ╚═
  `));
...
async function createInteractiveTheme(options: { skipIntro?: boolean } = {}) {
    ...
    colors = await inquirer.prompt([
      {
        ...
        transformer: (input) => chalk.hex(input || '#ffffff')(input || '#ffffff')
      },
      {
        type: 'input',
        name: 'error',
        ...
        default: '#ff4444',
        validate: (input) => /^#[0-9A-Fa-f]{6}$/.test(input) || 'Invalid hex color',
        transformer: (input) => chalk.hex(input || '#ff4444')(input || '#ff4444')
      },
      {
        type: 'input',
        name: 'warning',
        ...
        default: '#ff9900',
        validate: (input) => /^#[0-9A-Fa-f]{6}$/.test(input) || 'Invalid hex color',
        transformer: (input) => chalk.hex(input || '#ff9900')(input || '#ff9900')
      },
      {
        type: 'input',
        name: 'success',
        ...
        default: '#00cc66',
        validate: (input) => /^#[0-9A-Fa-f]{6}$/.test(input) || 'Invalid hex color',
        transformer: (input) => chalk.hex(input || '#00cc66')(input || '#00cc66')
      },
      {
        type: 'input',
        name: 'info',
        ...
        default: '#00aaff',
        validate: (input) => /^#[0-9A-Fa-f]{6}$/.test(input) || 'Invalid hex color',
        transformer: (input) => chalk.hex(input || '#00aaff')(input || '#00aaff')
      },
      {
        type: 'input',
        name: 'muted',
        ...
        default: '#666666',
        validate: (input) => /^#[0-9A-Fa-f]{6}$/.test(input) || 'Invalid hex color',
        transformer: (input) => chalk.hex(input || '#666666')(input || '#666666')
      }
    ]);
  }
  ...
  if (checkAccessibility) {
    ...
    const accessBox = boxen(
      ...
      (result.recommendations.length > 0 ? '\nRecommendations:\n' + result.recommendations.map(r => `• ${r}`).join('\n') : '\n✅ No issues found!'),
      {
        ...
        padding: 1,
        borderStyle: 'round',
        borderColor: result.level === 'FAIL' ? 'red' : result.level === 'A' ? 'yellow' : 'green'
      }
    );
  ...
  console.log(boxen(
    ...
    chalk.dim(`Use your theme with: ${chalk.cyan(`logsdx --theme ${theme.name}`)}`),
    {
      padding: 1,
      borderStyle: 'round',
      borderColor: 'green'
...
export function createThemeCommand() {
  ...
  theme
    ...
    .action((options) => {
      ...
      if (options.detailed) {
        ...
        themes.forEach(name => {
          const theme = getTheme(name);
          const icon = theme.mode === 'light' ? '☀️ ' : theme.mode === 'dark' ? '🌙' : '🔄';
  ...
  theme
    ...
    .action((name) => {
      ...
      // Show color palette
      const palette = boxen(
        Object.entries(theme.schema).map(([key, value]) => {
