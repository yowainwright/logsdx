...
export function getAdaptiveTheme(
  ...
  const highContrast = options?.respectHighContrast ? detectHighContrast() : false
...
export function useAdaptiveLogger(
  ...
  if (typeof window === 'undefined') {
    throw new Error('useAdaptiveLogger can only be used in browser environments')
  }
...
export function generateThemeProperties(theme: any): string {
  ...
  const colors = theme.colorPalette || theme.colors || theme.schema?.colorPalette
