...
export async function loadJsonRules(filePath?: string): Promise<LineParser> {
  ...
  if (filePath) {
    try {
      ...
      console.warn(`Failed to load custom JSON rules: ${JSON5.stringify(error)}`);
      console.info("Using default JSON rules instead");
    }
  }

  // Create a direct JSON parser using JSON5
  return (line: string): ParsedLog => {
    try {
