...
import { THEMES as SHARED_THEMES, DEFAULT_THEME } from "../../../../src/theme";
...
export class StyleManager {
  ...
  private constructor() {
    // Use the shared default theme if available, otherwise fall back to local default
    const defaultThemeName = DEFAULT_THEME || 'default';

    // Convert shared theme to local theme format if it exists
    if (SHARED_THEMES && SHARED_THEMES[defaultThemeName]) {
      const sharedTheme = SHARED_THEMES[defaultThemeName];

      // Create a compatible theme from the shared theme
      this.currentTheme = {
        name: sharedTheme.name,
        levels: { ...THEMES.default.levels },
        fields: { ...THEMES.default.fields },
        status: { ...THEMES.default.status },
        patterns: { ...THEMES.default.patterns },
        elements: { ...THEMES.default.elements }
      };

      // Copy colors from shared theme to local theme
      Object.entries(sharedTheme.levels).forEach(([level, config]) => {
        if (this.currentTheme.levels[level as LogLevel]) {
          this.currentTheme.levels[level as LogLevel].color = config.color;
          this.currentTheme.levels[level as LogLevel].bold = config.bold;
          this.currentTheme.levels[level as LogLevel].dim = config.dim;
        }
      });

      // Copy status colors from shared theme
      Object.entries(sharedTheme.status).forEach(([status, config]) => {
        if (this.currentTheme.status[status]) {
          this.currentTheme.status[status].color = config.color;
        }
      });
    } else {
      // Fall back to local default theme
  ...
  public setTheme(
    ...
    if (typeof themeName === "string") {
      // First check if it's in the shared themes
      if (SHARED_THEMES && themeName in SHARED_THEMES) {
        const sharedTheme = SHARED_THEMES[themeName];
        ...
        // Copy colors from shared theme to local theme
        Object.entries(sharedTheme.levels).forEach(([level, config]) => {
          if (newTheme.levels[level as LogLevel]) {
            newTheme.levels[level as LogLevel].color = config.color;
            newTheme.levels[level as LogLevel].bold = config.bold;
            newTheme.levels[level as LogLevel].dim = config.dim;
          }
        });

        // Copy status colors from shared theme
        Object.entries(sharedTheme.status).forEach(([status, config]) => {
          if (newTheme.status[status]) {
            newTheme.status[status].color = config.color;
          }
        });

        this.currentTheme = newTheme;
      }
      // Then check if it's a built-in theme
...
export const setTheme = (
  ...
  // Just delegate to the StyleManager which now handles shared themes
