...
const themes = ['github-light', 'github-dark', 'solarized-light', 'solarized-dark', 'dracula', 'oh-my-zsh']

console.log('Current theme modes:')
themes.forEach(name => {
  const theme = getTheme(name)
  console.log(`${name.padEnd(20)} mode: ${theme.mode || 'not specified'} (isLight: ${isLightThemeStyle(theme)})`)
})
...
console.log(`Custom dark theme - isLight: ${isLightThemeStyle(customDarkTheme)}`)
console.log(`Custom light theme - isLight: ${isLightThemeStyle(customLightTheme)}`)
...
const lightLogger = getLogsDX({ theme: customLightTheme, outputFormat: 'ansi' })
console.log('\nLight theme (with box):')
