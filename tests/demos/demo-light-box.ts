...
function createLightBox(title: string, lines: string[], width: number = 60) {
  ...
  console.log(
    BOX_TOP_LEFT +
      BOX_HORIZONTAL.repeat(titlePadding) +
      paddedTitle +
      BOX_HORIZONTAL.repeat(width - titlePadding - paddedTitle.length - 2) +
      BOX_TOP_RIGHT,
  );
  ...
  // Bottom border
  console.log(
    BOX_BOTTOM_LEFT + BOX_HORIZONTAL.repeat(width - 2) + BOX_BOTTOM_RIGHT,
  );
}
...
function createFullBlock(title: string, lines: string[], width: number = 70) {
  ...
  console.log(
    bg +
      "\x1b[38;2;0;0;0m" +
      ` ${title.toUpperCase()} THEME PREVIEW`.padEnd(width),
  );
  console.log(bg + "\x1b[38;2;0;0;0m" + " ".repeat(width));
  ...
  lines.forEach((line) => {
    ...
    process.stdout.write(
      " ".repeat(Math.max(0, padding)) + "  " + RESET_ALL + "\n",
    );
  });
