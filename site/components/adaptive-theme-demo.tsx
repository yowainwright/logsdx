...
import { getLogsDX, getAllThemes, getTheme, detectColorScheme, detectHighContrast, detectReducedMotion } from 'logsdx'
...
export function AdaptiveThemeDemo() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const terminalLogger = getLogsDX({ theme: themeName, outputFormat: 'html' })
  ...
  return (
    <Card className="w-full max-w-6xl">
      <CardHeader>
        ...
        <CardDescription>
          See how themes adapt to your system preferences with real logsdx processing
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <div className="space-y-2">
            ...
            <select
              value={outputFormat}
              onChange={(e) => setOutputFormat(e.target.value as 'ansi' | 'html')}
              className="px-3 py-2 border rounded-md"
            >
        ...
        <div className="p-4 bg-secondary rounded-md space-y-2">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-mono">{systemPreferences.highContrast ? 'enabled' : 'disabled'}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-medium">Reduced Motion:</span>
            <span className="font-mono">{systemPreferences.reducedMotion ? 'enabled' : 'disabled'}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-medium">Auto-detect Status:</span>
            <span className="font-mono">{autoDetect ? 'active' : 'manual'}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-medium">Theme Background:</span>
            <span className="font-mono" style={{ color: themeColors.background }}>{themeColors.background}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-medium">Theme Text Color:</span>
            <span className="font-mono" style={{ color: themeColors.text }}>{themeColors.text}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-muted-foreground">
              Showing {outputFormat === 'ansi' ? 'Terminal' : 'Browser'} output with real logsdx processing (both use HTML rendering for browser compatibility)
            </div>
          </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded ${
                    outputFormat === 'ansi' ? 'bg-primary text-primary-foreground' : 'bg-secondary'
                  }`}
                >
                  View
                </button>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded ${
                    outputFormat === 'html' ? 'bg-primary text-primary-foreground' : 'bg-secondary'
                  }`}
                >
                  View
                </button>
              ...
              >
                {logs.map((log, i) => (
                  <div key={`browser-${i}`} dangerouslySetInnerHTML={{ __html: log.html }} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-md border-2 border-primary/20 bg-primary/5">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-medium">
                Current Output: {outputFormat === 'ansi' ? 'Terminal' : 'Browser'}
              </h4>
              <span className="text-xs text-muted-foreground">Theme: {currentTheme}</span>
            </div>
            <div 
            ...
            >
              {logs.map((log, i) => (
                <div key={`current-${i}`} className="whitespace-pre-wrap">
                  <div dangerouslySetInnerHTML={{ __html: outputFormat === 'ansi' ? log.ansi : log.html }} />
                </div>
              ))}
            </div>
          </div>
  ...
  color: var(--logsdx-success);
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {/* Basic Usage */}
            <div>
              <h4 className="text-sm font-medium mb-2">Basic Adaptive Logger</h4>
...
`}
            ...
            {/* Advanced Usage */}
            <div>
              <h4 className="text-sm font-medium mb-2">Advanced Adaptive Features</h4>
              <pre className="p-3 bg-secondary rounded-md text-xs overflow-x-auto">
{`import { 
  AdaptiveLogger,
  detectHighContrast,
...
`}
          ...
          {/* Current Demo Code */}
          <div>
            <h4 className="text-sm font-medium mb-2">Current Demo Configuration</h4>
            <pre className="p-3 bg-secondary rounded-md text-xs overflow-x-auto">
