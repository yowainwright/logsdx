...
function getLogColor(text: string) {
  if (text.includes("[ERROR]")) return LOG_COLORS.error;
  if (text.includes("[WARN]")) return LOG_COLORS.warn;
  if (text.includes("[SUCCESS]")) return LOG_COLORS.success;
  if (text.includes("[INFO]")) return LOG_COLORS.info;
  if (text.includes("[DEBUG]")) return LOG_COLORS.debug;
  return LOG_COLORS.default;
}

function StyledLog({ log }: { log: DemoLog }) {
  const color = getLogColor(log.text);
  const fontWeight = log.text.includes("[ERROR]") ? "bold" : "normal";
...
function LogPane({
  ...
  const title = isBrowser ? "Browser Console" : "Terminal";
    ...
    <div className="p-4 flex flex-col">
      ...
      >
        {DEMO_LOGS.map((log, index) => (
          <LogEntry
            key={`${isBrowser ? "browser" : "terminal"}-${index}`}
...
function CodeEditor({ showWithLogsDx }: { showWithLogsDx: boolean }) {
  const code = showWithLogsDx ? WITH_LOGSDX : WITHOUT_LOGSDX;
...
function StatusBar({ showWithLogsDx }: { showWithLogsDx: boolean }) {
  const statusColor = showWithLogsDx ? "bg-green-500" : "bg-yellow-500";
  const statusText = showWithLogsDx ? "Styled" : "Plain";
  const summary = showWithLogsDx ? "logsDx enabled" : "Standard console";
  const statuses = [
    ["Terminal", statusText],
    ["Browser", statusText],
  ];
      ...
      <div className="flex gap-3">
        {statuses.map(([label, text]) => (
          <div className="flex items-center gap-1.5" key={label}>
            <div
              className={`w-2 h-2 rounded-full transition-colors ${statusColor}`}
            ></div>
...
function OutputPanes({
  activeIndex,
  showWithLogsDx,
}: Pick<LogPaneProps, "activeIndex" | "showWithLogsDx">) {
  return (
    <div className="grid grid-cols-2 divide-x divide-slate-700">
      <LogPane
        activeIndex={activeIndex}
        iconClassName="w-3 h-3 rounded-sm bg-slate-700"
        isBrowser={false}
        showWithLogsDx={showWithLogsDx}
      />
      <LogPane
        activeIndex={activeIndex}
        iconClassName="w-3 h-3 rounded-full bg-slate-700"
        isBrowser={true}
        showWithLogsDx={showWithLogsDx}
      />
    </div>
...
function DemoCard({
  ...
  const badge = showWithLogsDx ? "✨ With logsDx" : "❌ Without logsDx";
...
function ProblemPoints() {
  return (
    <ul className="space-y-3">
      ...
      <li className="flex items-start">
        <span className="mr-2 text-red-500">×</span>
        <span>Lost context when switching between terminal and browser</span>
      </li>
    </ul>
  );
}
    ...
    <div>
      ...
      <div className="space-y-6 text-lg text-slate-600 dark:text-slate-400">
        <p className="text-xl font-bold text-slate-900 dark:text-slate-100">
          Have you ever built a beautiful logging system for your terminal, only
          to have it look completely different in the browser console?
        </p>
        <p>
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
            ...
            <div className="relative lg:sticky lg:top-24">
              <DemoCard
                ...
                onToggle={() => setShowWithLogsDx((prev) => !prev)}
                showWithLogsDx={showWithLogsDx}
              />
            </div>
          </div>
